...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType) : LiveData<Course?> {
        val q = nearestQuery(queryType)
        return dao.getNearestSchedule(q)
    }

    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        val sort = sortedQuery(sortType)

        val getSort = dao.getAll(sort)

        val pagedListConf = PagedList.Config.Builder()
            .setPageSize(PAGE_SIZE)
            .build()

        return LivePagedListBuilder(getSort, pagedListConf)
            .build()
    }
